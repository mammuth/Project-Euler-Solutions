...
public class Problem011 {
    ...
    public static void main(String[] args) {
        ...
        for (int m = 0; m < 20; m++)
            for (int n = 0; n < 20; n++, i++)
                matrix[m][n] = Integer.parseInt(input[i]);
        ...
        for (int a = 0; a < 20; a++) {
            for (int b = 0; b < 20; b++) {
                ...
                // Check up, down, left, right & diagonally by starting at matrix[a][b]
                ...
                try {
                    ...
                    for (int lr = -3; lr <= 3; lr++) {
                        candidates[c] = matrix[a][b + lr];
                        c++;
                        if (lr == 0) {
                            compareToMaxProduct(candidates);
                    ...
                    candidates = new int[4];
                    c = 0;
                    for (int ud = -3; ud <= 3; ud++) {
                        candidates[c] = matrix[a + ud][b];
                        c++;
                        if (ud == 0) {
                            compareToMaxProduct(candidates);
                            candidates = new int[4];
    ...
    private static void compareToMaxProduct(int[] neighbours) {
        long tmpProduct = 1;
        for (int j = 0; j < neighbours.length; j++) {
            tmpProduct *= neighbours[j];
        }
